...
const MC = (() => {
  ...
  const SIMS = 10000;
  const RISK = 0.02;
  const LS   = 'mc_archive1_filter';  // localStorage key
  ...
  async function fetchCSV() {
    const names = ['Trade_Log-Archive-1.csv'];
    for (const name of names) {
      const res = await fetch(encodeURI(name));
      if (res.ok) return res.text();
    }
    throw new Error('CSV not found (tried Trade_Log-Archive-1.csv and Trade_Log-Archive-1.csv)');
  }
